...
* **Gate-Level Design:** Constructed explicitly using `not`, `and`, and `or` logic gates to implement the selection logic[cite: 9].
* **Selection Mechanism:** Inverts the `sel` signal to gate the `i0` input and uses the raw `sel` signal to gate the `i1` input, combining them to select the final output[cite: 9].
* **Why Gate-Level?** Demonstrates a deep understanding of underlying hardware logic and how higher-level mux operations are physically implemented on silicon[cite: 9].
...
* **Waveform Generation:** Dumps real-time signal transitions into a `muxGtime.vcd` file for logic analyzer inspection in GTKWave[cite: 10].