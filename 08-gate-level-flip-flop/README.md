...
* **Gate-Level Implementation:** Explicitly constructed using primitive `not`, `and`, and `nor` logic gates.
...
* **Clock & Data Sweeping:** Manually drives clock and data transitions to verify master-slave latch interactions.
* **Waveform Export:** Dumps signal execution into a `timingDiagram.vcd` file for logic analyzer review.